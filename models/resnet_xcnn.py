...
directory = './'
checkpoint_dir = directory+'ckpt/resnet_xcnn/'
...
class BasicBlock(nn.Module):
    expansion = 1
    def __init__(self, in_planes, planes, stride=1, cfg='CNN'):
        ...
        if self.cfg == 'CNN':
            ...
            self.conv1 = xCNN(in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
            self.conv2 = xCNN(planes, planes, kernel_size=3, stride=1, padding=1, bias=False)
        elif self.cfg == 'xCNNlow':
            self.conv1 = xCNNlow(in_planes, planes, kernel_size=3, stride=stride, padding=1, rank=rank, bias=False)
            self.conv2 = xCNNlow(planes, planes, kernel_size=3, stride=1, padding=1, rank=rank, bias=False)
        ...
        if stride != 1 or in_planes != self.expansion*planes:
            if self.cfg == 'CNN':
                ...
                self.shortcut = nn.Sequential(
                    xCNN(in_planes, self.expansion*planes, kernel_size=1, stride=stride, bias=False),
                    nn.BatchNorm2d(self.expansion*planes)
                )
            elif self.cfg == 'xCNNlow':
                self.shortcut = nn.Sequential(
                    xCNNlow(in_planes, self.expansion*planes, kernel_size=1, stride=stride, rank=rank, bias=False),
                    nn.BatchNorm2d(self.expansion*planes)
                )
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, cfg='CNN'):
        ...
        if self.cfg == 'CNN':
            ...
            self.conv1 = xCNN(in_planes, planes, kernel_size=1, bias=False)
            self.conv2 = xCNN(planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
            self.conv3 = xCNN(planes, self.expansion*planes, kernel_size=1, bias=False)
        elif self.cfg == 'xCNNlow':
            self.conv1 = xCNNlow(in_planes, planes, kernel_size=1, rank=rank, bias=False)
            self.conv2 = xCNNlow(planes, planes, kernel_size=3, stride=stride, padding=1, rank=rank, bias=False)
            self.conv3 = xCNNlow(planes, self.expansion*planes, kernel_size=1, rank=rank, bias=False)
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, cfg='CNN'):
        ...
        if self.cfg == 'CNN':
            ...
            self.conv1 = xCNN(3, 64, kernel_size=3, stride=1, padding=1, bias=False)
        elif self.cfg == 'xCNNlow':
            self.conv1 = xCNNlow(3, 64, kernel_size=3, stride=1, padding=1, rank=rank, bias=False)
    ...
    def _make_layer(self, block, planes, num_blocks, stride):
        ...
        for stride in strides:
            layers.append(block(self.in_planes, planes, stride, self.cfg))
...
net = ResNet18(netType).to(device)
