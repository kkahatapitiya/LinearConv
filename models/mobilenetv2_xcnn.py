...
directory = './'
checkpoint_dir = directory+'ckpt/mobilenetv2_xcnn/'
...
class Block(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, expansion, stride, cfg='CNN'):
        ...
        if cfg=='CNN':
            ...
            self.conv1 = xCNN(in_planes, planes, kernel_size=1, stride=1, padding=0, bias=False)
            self.conv2 = xCNN(planes, planes, kernel_size=3, stride=stride, padding=1, groups=planes, bias=False)
            self.conv3 = xCNN(planes, out_planes, kernel_size=1, stride=1, padding=0, bias=False)
        elif cfg=='xCNNlow':
            self.conv1 = xCNNlow(in_planes, planes, kernel_size=1, stride=1, padding=0, rank=rank, bias=False)
            self.conv2 = xCNNlow(planes, planes, kernel_size=3, stride=stride, padding=1, groups=planes, rank=rank, bias=False)
            self.conv3 = xCNNlow(planes, out_planes, kernel_size=1, stride=1, padding=0, rank=rank, bias=False)
        ...
        if stride == 1 and in_planes != out_planes:
            if cfg=='CNN':
                ...
                self.shortcut = nn.Sequential(
                    xCNN(in_planes, out_planes, kernel_size=1, stride=1, padding=0, bias=False),
                    nn.BatchNorm2d(out_planes),
                )
            elif cfg=='xCNNlow':
                self.shortcut = nn.Sequential(
                    xCNNlow(in_planes, out_planes, kernel_size=1, stride=1, padding=0, rank=rank, bias=False),
                    nn.BatchNorm2d(out_planes),
                )
...
class MobileNetV2(nn.Module):
    ...
    def __init__(self, num_classes=10, cfg='CNN'):
        ...
        if cfg=='CNN':
            ...
            self.conv1 = xCNN(3, 32, kernel_size=3, stride=1, padding=1, bias=False)
            self.conv2 = xCNN(320, 1280, kernel_size=1, stride=1, padding=0, bias=False)
        elif cfg=='xCNNlow':
            self.conv1 = xCNNlow(3, 32, kernel_size=3, stride=1, padding=1, rank=rank, bias=False)
            self.conv2 = xCNNlow(320, 1280, kernel_size=1, stride=1, padding=0, rank=rank, bias=False)

    def _make_layers(self, in_planes):
        layers = []
        for expansion, out_planes, num_blocks, stride in self.cfgx:
            strides = [stride] + [1]*(num_blocks-1)
            for stride in strides:
                layers.append(Block(in_planes, out_planes, expansion, stride, self.cfg))
...
net = MobileNetV2(cfg=netType).to(device)
